...
import type { LiveDriver, LiveInterval, LiveLap, LivePitStop, LivePosition, LiveRaceControlEvent, LiveRaceState, LiveSession, LiveStint, StreamHealth } from "@/lib/f1/domain/live";
import { mapDriver, mapInterval, mapLap, mapPitStop, mapPosition, mapRaceControl, mapSession, mapStint } from "./mappers";
...
type CachedStreams = { session: LiveSession | null; drivers: LiveDriver[]; position: LivePosition[]; intervals: LiveInterval[]; laps: LiveLap[]; stints: LiveStint[]; pit: LivePitStop[]; raceControl: LiveRaceControlEvent[] };
...
function emptyCache(): CachedStreams { return { session: null, drivers: [], position: [], intervals: [], laps: [], stints: [], pit: [], raceControl: [] }; }
...
export function createLiveStateLoader(provider: OpenF1LiveProvider) {
  ...
  async function getLiveRaceState(): Promise<LiveRaceState> {
    ...
    const [drivers, position, intervals, laps, stints, pit, raceControl] = await Promise.all([
      ...
      load("pit", async () => (await provider.getPitStops(sessionKey)).map(mapPitStop), cache.pit, POLLING_INTERVALS.pit, now),
