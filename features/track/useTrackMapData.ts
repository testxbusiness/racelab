...
type TrackMapSource = { replayFixtureId?: string | null; replayElapsedMs?: number };
...
export function useTrackMapData(sessionKey: number, coordinateBounds: LocationBounds = MONZA_LOCATION_BOUNDS, enabled = true, { replayFixtureId = null, replayElapsedMs = 0 }: TrackMapSource = {}): MapData {
  const [data, setData] = useState<MapData>({ samples: [], bounds: coordinateBounds, sourceTimestamp: null, status: "loading", error: null });
  const replayElapsedRef = useRef(replayElapsedMs);
  replayElapsedRef.current = replayElapsedMs;
  useEffect(() => {
    let active = true;
    ...
    let previousReplayElapsed = replayElapsedRef.current;
    const storageKey = `racelab:location-cursor:${sessionKey}`;
    if (!replayFixtureId) {
      try { cursor = window.sessionStorage.getItem(storageKey); } catch { cursor = null; }
    }
    if (!cursor && !replayFixtureId) cursor = new Date(Date.now() - 15_000).toISOString();
    const refresh = async () => {
      if (document.visibilityState !== "visible" || !navigator.onLine) return;
      try {
        const currentReplayElapsed = replayElapsedRef.current;
        if (replayFixtureId && currentReplayElapsed < previousReplayElapsed) {
          cursor = null;
          setData({ samples: [], bounds: coordinateBounds, sourceTimestamp: null, status: "loading", error: null });
        }
        previousReplayElapsed = currentReplayElapsed;
        const url = replayFixtureId
          ? `/api/replay/${replayFixtureId}/location?elapsedMs=${currentReplayElapsed}&after=${encodeURIComponent(cursor ?? "")}`
          : `/api/openf1/location?session_key=${sessionKey}&after=${encodeURIComponent(cursor ?? "")}`;
        ...
        if (nextTimestamp && (!cursor || nextTimestamp > cursor)) {
          cursor = nextTimestamp;
          if (!replayFixtureId) {
            try { window.sessionStorage.setItem(storageKey, cursor); } catch { /* storage is an optional optimization */ }
          }
        }
  ...
  }, [sessionKey, coordinateBounds, enabled, replayFixtureId]);
  return data;
}
