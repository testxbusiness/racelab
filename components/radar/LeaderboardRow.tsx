...
import { hasSectorTimes, SectorTimes } from "./SectorTimes";
...
export function LeaderboardRow({ driver, favourite, onSelect }: { driver: LiveDriverTiming; favourite: boolean; onSelect: () => void }) {
  ...
  return <button type="button" className={`leaderboard-row${favourite ? " leaderboard-row-favourite" : ""}`} style={teamStyle} onClick={onSelect} aria-label={`Open ${driver.driver.fullName} driver focus${favourite ? ", favourite driver" : ""}`}><span className="team-accent" aria-hidden="true" /><span className="position">{driver.position ?? "—"}</span><div className="driver-cell"><strong>{favourite ? "★ " : ""}{driver.driver.acronym}</strong><span className="driver-team-meta"><TeamLogo teamName={driver.driver.teamName} />{driver.driver.teamName ?? "Team unavailable"}</span></div><div className="gap-cell"><strong>{timingValue(driver.gapToLeader)}</strong><span>{driver.position === 1 ? "LEADER" : `INT ${timingValue(driver.interval)}`}</span></div><TyreBadge compound={driver.compound} age={driver.tyreAge} /><div className="row-status"><span>PITS {driver.pitStops ?? "—"}</span>{driver.inPit ? <span>PIT</span> : null}{driver.retired ? <span>RETIRED</span> : null}</div>{hasSectorTimes(driver) ? <SectorTimes sector1Seconds={driver.sector1Seconds} sector2Seconds={driver.sector2Seconds} sector3Seconds={driver.sector3Seconds} /> : null}</button>;
}
