...
function refreshStatusTone({ cachedAt, error, offline, refreshing, retryAttempt, lifecycle }: { cachedAt: string | null; error: string | null; offline: boolean; refreshing: boolean; retryAttempt: number; lifecycle?: LiveRaceState["lifecycle"] }): "live" | "delayed" | "offline" | "final" | "scheduled" {
  if (lifecycle === "ENDED" || lifecycle === "FINALIZING") return "final";
  if (lifecycle === "UPCOMING") return "scheduled";
  if (offline || cachedAt) return "offline";
  if (error || refreshing || retryAttempt > 0) return "delayed";
  return "live";
}
...
function refreshMessage({ cachedAt, error, offline, refreshing, retryAttempt, nextRetryAt, lowDataMode, lifecycle }: { cachedAt: string | null; error: string | null; offline: boolean; refreshing: boolean; retryAttempt: number; nextRetryAt: number | null; lowDataMode: boolean; lifecycle?: LiveRaceState["lifecycle"] }): string {
  if (lifecycle === "ENDED") return "FINAL TIMING · LIVE POLLING DISABLED";
  if (lifecycle === "FINALIZING") return "FINALIZING · WAITING FOR FINAL TIMING";
  if (lifecycle === "UPCOMING") return "UPCOMING SESSION · LIVE POLLING DISABLED";
