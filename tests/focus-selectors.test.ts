...
const driver = (number: number, position: number, gapToLeader: number): LiveDriverTiming => ({ driver: { number, fullName: `Driver ${number}`, acronym: `D${number}`, teamName: "Team", teamColour: null }, position, gapToLeader, interval: position === 1 ? 0 : 2.5, compound: "MEDIUM", tyreAge: 8, lastLapSeconds: 81.2, bestLapSeconds: 80.9, inPit: null, retired: null, sourceTimestamp: null });
