...
describe("location service", () => {
  it("passes the last location timestamp cursor to the provider and returns a snapshot", async () => {
    ...
    const service = createLocationService({ getLocations: async (_sessionKey, after) => { receivedAfter = after ?? null; return [{ date: "2026-08-23T12:00:01+00:00", driver_number: 16, session_key: 1, x: 10, y: 20, z: 30 }]; } });
